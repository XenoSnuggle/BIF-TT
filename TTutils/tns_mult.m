...
tmpa = zeros(1,nA);
tmpa(aid) = 1;
are = sort(find(tmpa==0),'ascend'); % dims of A \ aid

tmpb = zeros(1,nB);
tmpb(bid) = 1;
bre = sort(find(tmpb==0),'ascend'); % dims of B \ bid

A = permute(A, [are, aid]);
nta = size(A);
nta1 = nta(1:numel(are));    
nta2 = nta((numel(are)+1):end);
A = reshape(A, prod(nta1),prod(nta2));

B = permute(B, [bid, bre]);
ntb = size(B);
ntb1 = ntb(1:numel(bid));    
ntb2 = ntb((numel(bid)+1):end);
